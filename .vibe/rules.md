...
- **Unity 验证方式**：每次代码改动后，不得主动关闭、重启或新开 Unity Editor。仅对当前已打开的项目执行 `AssetDatabase.Refresh`（优先使用 `assets-refresh` / 对应 MCP Refresh 能力）并等待脚本编译结果；若当前没有已打开的 Unity Editor，必须报告“无法按当前会话规则刷新验证”，不得自行启动 Unity。
- **工作树归属**：七宗罪被动、罪印、Boss 及其配套系统的实现统一落在主项目工作树 `Kepler`。除非用户明确要求并行实验或隔离验证，不再为这些需求新建额外 Git worktree；已有隔离 worktree 的改动应先审计，再按文件和资产引用安全汇总回 `Kepler`，不得直接覆盖主工作树中的用户改动。
