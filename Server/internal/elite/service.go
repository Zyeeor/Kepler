...
//   - seed.go：种子快照注入
//   - userbd.go：用户 BD 工具在线上传（严格校验 + 原子存文件）
//   - userbd_import.go：用户 BD 启动目录导入（宽松跳过）
//   - fingerprint.go：BD 内容指纹缓存与去重工具
//   - upload.go：客户端滚动上传 + 容量治理
//   - pick.go：四步筛选 + 三级兜底 + TOP_BAND 加权随机
//   - events.go：战果回传聚合 + 战绩查询
//   - leaderboard.go：荣誉殿堂排行榜（含进程内缓存）
//   - dedup.go：战果事件幂等去重
package elite
...
type EliteService struct {
	...
	// 战果回传 reporter 累计配额（events.go）：进程内计数，重启清零（与 eventId 去重
	// 同语义的最终一致防护）；防单个 reporter 持续灌入伪造聚合。
	repMu   sync.Mutex
	repSeen map[string]int
